...
_HANDLERS: dict[str, Handler] = {
    ...
    "list_catalogs": lambda session, _a: session.execute("SELECT DISTINCT catalog_name FROM information_schema.schemata ORDER BY catalog_name"),
    "list_databases": lambda session, _a: session.execute("SELECT database_name, path, type, readonly FROM duckdb_databases() WHERE NOT internal ORDER BY database_name"),
